...
export const MarketplacePortalBar = ({ selected, onChange }: PortalIconBarProps) => {
  return (
    <div className="w-full overflow-hidden border-b border-[var(--pm-border)] bg-[var(--pm-bg)]">
      <div className="flex items-center gap-4 px-4 py-4 overflow-x-auto no-scrollbar scroll-smooth">
        {portals.map((portal) => {
            ...
            >
              <div className={`
                ...
                ${isActive 
                  ...
                  : 'bg-[var(--pm-surface)] border border-[var(--pm-border)] group-hover:border-white/20'
                }
              `}>
                <Icon className={`w-6 h-6 ${isActive ? 'text-white' : 'text-zinc-400 group-hover:text-white'}`} />
              </div>
              <span className={`
              ...
              {isActive && (
                <motion.div 
                  ...
                  className="w-8 h-0.5 bg-cyan-400 rounded-full"
                />
              )}
            </button>
          );
