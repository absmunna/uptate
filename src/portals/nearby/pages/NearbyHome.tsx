...
export default function NearbyHome() {
  ...
  const { data: shops, isLoading } = useQuery({
    ...
    queryFn: () => fetch('/api/v1/vendors?type=nearby').then(res => res.json())
  });

  const filtered = shops?.filter((p: any) => 
    ...
    <div className="pb-16 w-full max-w-[480px] mx-auto min-h-screen">
      ...
      <div className="px-3 mb-3">
        <div className="flex items-center gap-2 rounded-2xl px-3.5 py-2.5 border"
          ...
          <input type="text" placeholder="Search local shops..." value={search}
