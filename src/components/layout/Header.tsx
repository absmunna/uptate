...
import {
  ...
  Languages, MapPin, Loader2, Navigation,
} from "lucide-react";
...
import { ThemeSwitcher, ThemeToggleBtn } from "@/components/theme/ThemeSwitcher";
...
function IconBtn({
  ...
  if (href) return <Link href={href}><div className={cls}>{inner}</div></Link>;
...
function SettingsPanel({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div
      ref={ref}
      className="absolute right-0 top-full mt-2 w-72 rounded-2xl border border-border glass-panel shadow-2xl z-[200] p-4 flex flex-col gap-4"
    >
      <ThemeSwitcher onClose={onClose} />
      <div className="h-px bg-border" />
      <div className="flex flex-col gap-2">
        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{t("lang.title")}</p>
...
export function Header() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) setLocation(`/marketplace?q=${encodeURIComponent(searchQuery)}`);
  };

  const isHome = location === "/";
  const isInner = !isHome && !location.startsWith("/marketplace");
      ...
      <header className="fixed top-0 left-0 right-0 z-50 w-full glass-panel border-b border-foreground/[0.07]">
        ...
        <div className="flex h-[52px] items-center px-3 md:px-5 gap-2 max-w-[1400px] mx-auto">
          ...
          <div className="flex items-center gap-0.5 shrink-0">
            ...
            {/* Create */}
            <Link href={role === "seller" || role === "admin" ? "/seller/products/new" : "/auth/seller-register"} className="hidden sm:flex ml-1">
            ...
            <button
              ...
              className="hidden md:flex items-center gap-1.5 ml-1 px-2 h-8 rounded-xl bg-foreground/[0.06] border border-foreground/[0.09] hover:bg-foreground/[0.1] text-foreground/55 hover:text-foreground text-[11px] font-bold transition-all"
            >
            ...
            {/* Avatar */}
            <Link href="/profile" className="ml-1.5">
        ...
        <div className="border-t border-foreground/[0.05]">
          <div className="flex items-center overflow-x-auto no-scrollbar px-3 md:px-5 h-10 gap-5 md:gap-6 text-[13px] font-medium max-w-[1400px] mx-auto">
            {TABS.map((tab) => {
              ...
              return (
                <Link
                  ...
                  href={tab.href}
                  className={cn(
                    "shrink-0 pb-2 pt-1 border-b-2 transition-all duration-200 whitespace-nowrap",
                    active
